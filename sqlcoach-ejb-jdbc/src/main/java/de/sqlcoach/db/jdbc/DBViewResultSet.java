...
import de.sqlcoach.db.jdbc.DBViewResultSet;
import de.sqlcoach.model.ViewResultRow;
import de.sqlcoach.model.ViewResultSet;
import de.sqlcoach.model.ViewResultValue;
...
public class DBViewResultSet {
  ...
  public static ViewResultSet get(Connection cn, String query) throws SQLException {
    ...
    final String command = (query != null) 
                           ? (query.length() > 6 ) 
                             ? query.substring(0,6).toUpperCase() : null
                           : null;
    
    if (command == null || !command.equals("SELECT")) {
