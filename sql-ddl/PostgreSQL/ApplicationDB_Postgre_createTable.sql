...
CREATE TABLE scenario (
  ...
  datecreate    TIMESTAMP(0) Not null,
  datelastmod   TIMESTAMP(0) Not null,
  FOREIGN KEY (app_user_id) REFERENCES app_user ON DELETE CASCADE,
  PRIMARY KEY (id),
  UNIQUE (description)
);
...
CREATE TABLE taskgroup (
  ...
  datecreate    TIMESTAMP(0) Not null,
  datelastmod   TIMESTAMP(0) Not null,
  FOREIGN KEY (scenario_id) REFERENCES scenario ON DELETE CASCADE,
  PRIMARY KEY (id)
);
...
CREATE TABLE task (
  ...
  datecreate    TIMESTAMP(0) Not null,
  datelastmod   TIMESTAMP(0) Not null,
  FOREIGN KEY (taskgroup_id) REFERENCES taskgroup ON DELETE CASCADE,
  PRIMARY KEY (id)
);
...
CREATE TABLE app_statistic (
  ...
  success      char(1)       CHECK (success IN ('0','1')) NOT NULL,
  query        varchar(2048) NOT NULL,
